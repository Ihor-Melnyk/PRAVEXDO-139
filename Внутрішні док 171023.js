...
/*	
function onCreate(){	
	EdocsApi.setAttributeValue({code:'Level'});
...
function onChangeLevel(){	
	var levelAttr = EdocsApi.getAttributeValue('Level');
		var isLevelVisible = levelAttr && (levelAttr.value=='24'||levelAttr.value=='29');
...
function setLevelEnValue(levelAttr){
	if(levelAttr && levelAttr.value){
		...
		var text = EdocsApi.findElementByProperty('code','kb3v',dic.attributes).value;
...
function setDirector(levelAttrValue){
	...
	if(levelAttrValue==='24'||levelAttrValue==='29'){
		var unitEmployee=EdocsApi.findEmployeeSubdivisionByLevelAndEmployeeID(CurrentUser.employeeId,2);
		if(unitEmployee && unitEmployee.managerId){
			var manager = EdocsApi.getEmployeeDataByEmployeeID(unitEmployee.managerId);
			if(manager){
				...
				director.push({
					...
					positionName:manager.positionName
									...
									EdocsApi.setAttributeValue({code: 'InitiatorHeadName', value: unitEmployee.managerName});
									EdocsApi.setAttributeValue({code: 'InitiatorHeadPosition', value: manager.positionName });
					directorValue = '[{"id":0,"employeeId":"'+ unitEmployee.managerId + '","index":0,"employeeName":"' + unitEmployee.managerName + '","positionName":"' + manager.positionName + '"}]';
	...
	EdocsApi.setAttributeValue({code: 'Director', value: directorValue, text:directorText});
}
...
function setControlforSecretary() {
   ...
   var Details2 = EdocsApi.getControlProperties('Details2');
    ...
    if (stage) {
        condition = stage.state == 'assigned' ||
...
debugger;
    if (stage) {
		var condition = stage.state == 'assigned' ||
            stage.state == 'inProgress' ||
            stage.state == 'completed' ||
            stage.state == 'delegated';
		if(condition){
			EdocsApi.setControlProperties({ code: "Responsible", hidden: false, required: true});
...
function setControlforProps() {
    ...
    var stage = EdocsApi.getCaseTaskDataByCode('SendEmail'+(Responsible ? JSON.parse(Responsible)[0]?.employeeId : ''));
...
var isContralHidden=!isVisible;
	EdocsApi.setControlProperties({ code: "Director", hidden: isContralHidden, required: true});
	EdocsApi.setControlProperties({ code: "Signatories1", hidden: isContralHidden, required: false});
if (isContralHidden){
...
function ldsSave(to,RegNumber,m, Description1,DOCTITLE, Level, Levell, Details, Details2, explan,Reason, info, KeyAmend, Link, persons , СontactPerson , Time1 ) {
...
debugger;
var subject =Description1+ ' №'+RegNumber.value + '  від '+ m.format("DD.MM.YYYY") ;
var doc = { 
		recipients: to, 
		subject: subject
	} ;
...
if (bodyAttr ){
	...
	//var methodRespons = EdocsApi.sendMsgMail(doc );
	debugger;
...
function onTaskExecuteSendEmail(routeStage) { 
 ...
 //function onTaskExecutedSendEmail(routeStage) { 
	//if(routeStage.executionResult != 'rejected') { 
  //   EdocsApi.sendJsonToLsdSystem('sendMail'); 
//
	//	}
//}
...
function saveJson() {
	...
	if(personAttr && personAttr.value){
		...
		for(var i =0;i<employees.length;i++){
			var employee   = EdocsApi.getEmployeeDataByEmployeeID(employees[i].employeeId );
			if(employee.email){
				person.push('<li><span>' + employee.shortName + '</span>&nbsp;<a href = "mailto: ' + employee.email + '">(' + employee.email + ')</a></li>');
			}else {
	...
	ldsSave(Emails, RegNumber, m, Descript, Doctitle, Level ,Levell, Details, Details2, explan,Reason, info.join('').replace('\n',''), infoEn.join('').replace('\n',''), Link, persons, personEn, timeValue.format("DD.MM.YYYY"));
...
debugger;
			var attributes = EdocsApi.getDictionaryItemData(item.dictionaryCode, item.id).attributes;
			var mails = EdocsApi.findElementByProperty('code','_x0076_qv9',attributes).value;
			return mails;
		});
...
function onChangeEmployee() {
    ...
    if (emp.value) {
        ...
        while (unitLevel >= 0) {
            try {
                var head = EdocsApi.getEmployeeManagerByEmployeeID(emp.value, unitLevel);
...
request.filterCollection.push({attributeCode:'Title', value:Type.text});
  ...
  var UnitEmployee=EdocsApi.findEmployeeSubdivisionByLevelAndEmployeeID(Person,0);
  var Status = EdocsApi.getControlProperties('Status');
 ...
 EdocsApi.setControlProperties(Status);
  ...
  function setEmployeeUnit(){
	  ...
	  if(employeeUnit){
		  ...
		  if(!employeeUnit.value){
			   ...
			   EdocsApi.setControlProperties({ code: "EmployeeUnit", hidden: false, required: false});
		  }
	  }
  }
  function checkVisibleEmployeeUnit(){
	  ...
	  if(employeeUnit){
		  if(!employeeUnit.value){
			  EdocsApi.setControlProperties({ code: "EmployeeUnit", hidden: true, required: false});
		  }
	  }
  }
  ...
  function onChangePerson(){
	  ...
	  if(person && person.value){
		  ...
		  for (var i = 0; i < employees.length; i++) {
		   var emploee = EdocsApi.getEmployeeDataByEmployeeID(employees[i].employeeId);
...
var text = EdocsApi.findElementByProperty('code', 'b6ug', dic.attributes).value;
conf2.value = text;
EdocsApi.setAttributeValue(conf2);
}
}
/*function onUnitMails() {
var unitmails = EdocsApi.getAttributeValue( 'unitmails');
var unitList = EdocsApi.getAttributeValue('UnitList');
 if (unitmails.value) {

var dic = EdocsApi.getDictionaryItemData('unitmails', unitmails.value);

var text = EdocsApi.findElementByProperty('code', 'ea3g', dic.attributes).value;
unitList.value = text;
...
var isContralHidden=!isVisible;
	EdocsApi.setControlProperties({ code: attributeCode, hidden: isContralHidden, required: required});
if (isContralHidden){
...
var unitmailsAttr = EdocsApi.getAttributeValue('unitmails').value;
	if(unitmailsAttr){
		...
		for(var i=0;i<unitmails.length;i++){
			...
			var name = EdocsApi.findElementByProperty('code', 'ea3g', dic.attributes).value;
